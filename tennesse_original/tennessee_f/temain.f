...
C
      STOP
C   Fechando o arquivo no final do programa principal
      CLOSE(UNIT=111)
C
      END
C
...
C
      ...
      COMMON/PV/ XMEAS(41), XMV(12)
C
C   Declaração de variáveis para manipulação do arquivo
      INTEGER :: ios

C   Abrindo o arquivo de saída (escrita contínua)
      OPEN(UNIT=111, FILE='./main_output.dat', STATUS='old', IOSTAT=ios)
      IF (ios /= 0) THEN
         PRINT *, 'Erro ao abrir o arquivo main_output.dat'
         RETURN
      END IF

C   Escrevendo no arquivo
      WRITE(111, 100) XMEAS(9), XMEAS(15), XMV(8)
